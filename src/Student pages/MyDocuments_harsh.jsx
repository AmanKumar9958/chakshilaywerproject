...
const MyDocuments = () => {
  ...
  return (
    ...
    >
      ...
      {/* Action Buttons: View and Analyze only */}
      <div className="flex gap-2 pt-3 border-t" style={{
        borderColor: `rgba(${parseInt(colors.golden.slice(1, 3), 16)}, ${parseInt(colors.golden.slice(3, 5), 16)}, ${parseInt(colors.golden.slice(5, 7), 16)}, 0.15)`
      }}>
        <button
          className="flex-1 flex items-center justify-center gap-1 px-3 py-2 rounded-lg font-semibold text-xs transition-all"
          style={{
      ...
      {/* Upload Modal - improved styling */}
{showUploadModal && (
  <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black bg-opacity-50 p-6">
    <div 
      ref={modalRef} 
      className="bg-white rounded-2xl shadow-xl max-w-3xl w-full max-h-[80vh] overflow-y-auto flex flex-col"
    ...
    >
      ...
      <div className="flex justify-between items-center p-6 border-b border-gray-200 rounded-t-2xl">
        <h2 className="text-2xl font-extrabold text-gray-900">Upload New Document</h2>
        <button onClick={() => { setShowUploadModal(false); resetForm(); }} className="text-gray-500 hover:text-gray-800 transition rounded-full p-2">
          <X className="w-6 h-6" />
        </button>
      </div>

      {/* Scrollable Form */}
      <form id="uploadForm" onSubmit={handleSubmit} className="space-y-6 p-6" style={{ color: '#2F3E46' }}>
        
        {/* File Upload Section */}
        <div>
          <label className="block text-sm font-semibold mb-2">File Upload <span className="text-amber-500">*</span></label>
          <div className="border-2 border-dashed border-amber-300 rounded-2xl p-8 text-center cursor-pointer">
            <input 
              ref={fileInputRef}
              id="file-upload" 
              required 
            ...
            <label htmlFor="file-upload" className="cursor-pointer">
              <Upload className="mx-auto mb-4 w-14 h-14 text-amber-600" />
              {newDocument.file ? (
                <>
                  <p className="font-semibold text-amber-800">{newDocument.file.name}</p>
              ...
              <p className="text-xs mt-1 text-gray-400">PDF, DOC, DOCX, JPG, PNG (Max 10MB)</p>
            </label>
          </div>
        </div>
        ...
        {/* Document Title */}
        <div>
          <label className="block text-sm font-semibold mb-2">Document Title <span className="text-amber-500">*</span></label>
          <input 
            required 
            name="title" 
            ...
            className="w-full border border-gray-300 rounded-xl p-3 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400"
          />
        </div>

        {/* Document Type */}
        <div>
          <label className="block text-sm font-semibold mb-2">Document Type <span className="text-amber-500">*</span></label>
          <select
            name="documentType"
            required
        ...
        {/* Description */}
        <div>
          <label className="block text-sm font-semibold mb-2 text-gray-600">Description <span className="text-xs">(Optional)</span></label>
          <textarea 
            name="description" 
        ...
        {uploading && (
          <div className="flex items-center space-x-3">
            <div className="w-full bg-gray-200 rounded-lg h-2 overflow-hidden">
              <div style={{ width: `${uploadProgress}%` }} className="h-full bg-amber-400 transition-all duration-300" />
            </div>
        ...
        <div className="flex justify-end gap-3">
          <button 
            ...
            className="px-6 py-2 rounded-xl border border-amber-400 text-amber-600 hover:bg-amber-50 transition"
          >
            Cancel
          </button>
          <button
            ...
            type="submit" 
            form="uploadForm"
            className="px-6 py-2 rounded-xl bg-gradient-to-r from-amber-400 to-amber-600 text-white font-semibold hover:from-amber-500 hover:to-amber-700 transition"
          >
