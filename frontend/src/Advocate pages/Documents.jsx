...
const DocumentsPage = () => {
 ...
 const handleContractComparison = async () => {
  ...
  try {
    // Create FormData for real n8n webhook
    ...
    const response = await axios.post(
      ...
      'https://n8n.srv983857.hstgr.cloud/webhook/620edd24-93aa-4567-b5d5-94caf2a5574d',
      formData,
      {
        headers: {
    ...
    // Transform n8n response to match your existing format
    const transformedResult = {
      status: 'success',
      documents: result.documents,
      comparison: {
        summary: result.comparison.summary,
        differences: result.comparison.detailedDifferences.map(diff => ({
          type: diff.type,
          severity: diff.type === 'removed' ? 'high' : diff.type === 'added' ? 'medium' : 'low',
          description: `Line ${diff.lineNumber}: ${diff.document1} → ${diff.document2}`,
          location: `Line ${diff.lineNumber}`,
          recommendation: `Review ${diff.type} content`,
          impact: diff.type === 'modified' ? 'Medium' : 'Low',
          clause: `Line ${diff.lineNumber}`
        })),
        keyChanges: result.comparison.keyChanges,
        metadata: {
          document1: result.documents.document1.filename,
          document2: result.documents.document2.filename,
          comparisonDate: result.timestamp,
          tool: 'n8n Document Comparison API',
          totalDifferences: result.comparison.summary.totalDifferences,
          similarity: result.comparison.summary.similarity
        }
      }
    };
    
    setResults(prev => ({ ...prev, comparison: transformedResult }));
    ...
    addNotification(`Comparison completed: ${result.comparison.summary.totalDifferences} differences found (${result.comparison.summary.similarity} similarity)`, 'success');
    
    console.log('n8n Comparison Result:', result);
  } catch (error) {
    console.error('Comparison error:', error);
    addNotification('Contract comparison failed: ' + error.message, 'error');
  } finally {
